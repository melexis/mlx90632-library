...
/** Test start extended measurement with data ready.
 */
void test_start_measurement_extended_success(void)
{
    uint16_t reg_status_mock = 0xCF; // cycle position 19 & data ready
    ...
    TEST_ASSERT_EQUAL_INT(19, mlx90632_start_measurement_extended());
}

/** Test start extended measurement function, if data is ready and if it is not it waits one usleep period before retry.
 *
 * First we simulate data not ready with bit0 not set. After one period we flip the bit0 to 1 to indicate
 * data ready and start measurement should complete with success.
 */
void test_start_measurement_extended_wait_ready(void)
{
    uint16_t reg_status_mock = 0xCC; // cycle position 19 & data NOT READY
    uint16_t reg_status_mock1 = 0xCF; // cycle position 19 & data READY
...
/** Test start extended measurement function, if data is ready but not all measurements are done. If it is not
 * all measurements are done, it waits one usleep period before retry.
 *
 * First we simulate data ready with cycle position set to 17. After one period we change the cycle position to 18.
 * After one period we change the cycle position to 19 and start measurement should complete with success.
 */
void test_start_measurement_extended_wait_full_ready(void)
{
    uint16_t reg_status_mock = 0xC7; // cycle position 17 & data READY
    uint16_t reg_status_mock1 = 0xCB; // cycle position 18 & data READY
    uint16_t reg_status_mock2 = 0xCF; // cycle position 19 & data READY
...
/** Test different failure paths in start_measurement_extended */
void test_start_measurement_extended_busy_i2c(void)
{
    uint16_t reg_status_mock = 0x46;
    ...
    TEST_ASSERT_EQUAL_INT(-EPERM, mlx90632_start_measurement_extended());
...
/** Test sensor timeouts while start_measure_extended due to lack of data ready.
 *
 * If this happens in real life it means that after 100 tries sensor still did not indicate full data set ready, which probably
 * points to much larger problem than a simple timeout. Timeout is only valid if usleep is a lot shorter than default
 * values
 */
void test_start_measurement_ready_timeout(void)
{
    uint16_t reg_status_first_mock = 0x46;
    uint16_t reg_status_mock[100];
    int i;

    for (i = 0; i < 100; ++i)
        reg_status_mock[i] = 0x0046; // cycle position 1 & data NOT READY through whole array!
    ...
    TEST_ASSERT_EQUAL_INT(-ETIMEDOUT, mlx90632_start_measurement_extended());
}

/** Test sensor timeouts while start_measure_extended due to not reaching the final measurement.
 *
 * If this happens in real life it means that after 100 tries sensor still did not indicate full data set ready, which probably
 * points to much larger problem than a simple timeout. Timeout is only valid if usleep is a lot shorter than default
 * values
...
void test_start_measurement_ready_timeout(void)
{
    uint16_t reg_status_first_mock = 0x47;
    uint16_t reg_status_mock[100];
    int i;

    for (i = 0; i < 100; ++i)
        reg_status_mock[i] = 0x0047; // cycle position 1 & data NOT READY through whole array!
